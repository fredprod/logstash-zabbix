...
class LogstashServer(protobix.SampleProbe):
    ...
    def _nodes_stats(self, url):
        ...
        # Process global events metrics
        zbx_key = 'logstash.pipeline.events.{0}'
        for events_section in nodes_stats['events']:
            if not isinstance(nodes_stats['events'][events_section], dict):
                data[zbx_key.format(events_section)] = nodes_stats['events'][events_section]
            else:
